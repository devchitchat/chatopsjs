export const page = {
  ...
  description: 'Ready-to-use chatops.js patterns: deployments, incident management, ticket tracking, access provisioning, and status checks.',
  nav: 'use-cases',
  hero: false,
  content: `
<div class="page-header">
  <div class="page-eyebrow">Field Manual · Common Missions</div>
  <h1 class="page-title">Common Use Cases</h1>
  <p class="page-subtitle">
    Production-ready patterns for the most common ChatOps workflows.
    Each example is a drop-in module you can extend for your environment.
  </p>
</div>
...
<nav style="background:var(--bg-surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px 24px;margin-bottom:40px">
  <div style="font-family:var(--font-mono);font-size:0.65rem;text-transform:uppercase;letter-spacing:0.18em;color:var(--text-faint);margin-bottom:12px">Mission Index</div>
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px">
    <a href="#deployments">🚀 Deployments</a>
    <a href="#incidents">🚨 Incident Management</a>
    <a href="#tickets">🎫 Ticket Tracking</a>
    <a href="#access">🔑 Access Provisioning</a>
    <a href="#status">📊 Status Checks</a>
    <a href="#secrets">🔒 Secret Rotation</a>
  </div>
</nav>

<h2 id="deployments">Deployments</h2>

<p>Trigger, monitor, and roll back deployments with a full confirmation flow and audit trail:</p>

<pre data-lang="js"><code>// modules/deploy.js
...
export default async function (robot) {
  ...
  robot.commands.register(new Command({
    ...
    confirm: {
      mode: 'yes-no',
      message: '🚀 Confirm deployment? (yes/no)'
    },
    handler: async (ctx) =&gt; {
      ...
      // Fire-and-forget: call your real deploy API here
      // e.g. await fetch('https://ci.internal/deploy', { method: 'POST', ... })

      return createMessageResponse({
        fallbackText: \`Deploying \${service} \${version} to \${environment}\`,
        blocks: [
          { type: 'section', text: \`🚀 **Deployment started** [\${deployId}]\` },
          { type: 'facts',   items: [
  ...
  robot.commands.register(new Command({
    ...
    confirm: {
      mode:    'yes-no',
      message: '⚠️ Roll back the last deployment? (yes/no)'
    },
    handler: async (ctx) =&gt; {
      const last = await ctx.storage.get('last-deploy')
      if (!last) return createTextResponse('No deployment to roll back.')

      // Call your rollback API here
      return createTextResponse(
        \`Rolling back \${last.service} \${last.version} from \${last.environment}…\`
      )
    }
  }))
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      return createMessageResponse({
        ...
        blocks: [
          { type: 'section', text: '📊 **Last deployment**' },
          { type: 'facts',   items: [
...
<p>Declare, acknowledge, update, and resolve incidents — with a status board and paging integration:</p>
...
export default async function (robot) {
  ...
  robot.commands.register(new Command({
    ...
    confirm: {
      mode:    'yes-no',
      message: '🚨 This will page on-call. Declare incident? (yes/no)'
    },
    handler: async (ctx) =&gt; {
      ...
      // TODO: ctx.storage.set / PagerDuty / OpsGenie API call here

      return createMessageResponse({
        fallbackText: \`🚨 \${id}: \${title}\`,
        blocks: [
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      return createTextResponse(\`🟢 \${ctx.args.id} resolved.\`)
    }
  }))
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      if (!open.length) return createTextResponse('No open incidents. 🟢')

      const lines = open.map(i =&gt; \`• **\${i.id}** [\${i.severity.toUpperCase()}] \${i.title}\`)
      return createTextResponse(lines.join('\\n'))
    }
  }))
}</code></pre>

<h2 id="tickets">Ticket Tracking</h2>

<pre data-lang="js"><code>// modules/tickets.js
import { Command, createTextResponse, createMessageResponse } from '@devchitchat/chatopsjs'
...
export default async function (robot) {
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      await ctx.storage.set('tickets', all)
      return createTextResponse(\`✅ \${ctx.args.id} closed.\`)
    }
  }))
}</code></pre>

<h2 id="access">Access Provisioning</h2>

<pre data-lang="js"><code>// modules/access.js
import { Command, createTextResponse, createMessageResponse } from '@devchitchat/chatopsjs'
...
export default async function (robot) {
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      // Call your real IAM / LDAP / Okta API here
      // await grantAccess({ user, service, role })
      ...
      return createMessageResponse({
        fallbackText: \`Access granted: \${user} → \${service}:\${role}\`,
        blocks: [
          { type: 'section', text: \`🔑 **Access granted**\` },
          { type: 'facts',   items: [
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      // await revokeAccess({ user, service })

      return createTextResponse(\`✅ Access revoked: \${user} from \${service}\`)
    }
  }))
}</code></pre>

<h2 id="status">Status Checks</h2>

<pre data-lang="js"><code>// modules/status.js
import { Command, createTextResponse, createMessageResponse } from '@devchitchat/chatopsjs'
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      if (service) {
        ...
        return createTextResponse(
          \`\${result.ok ? '🟢' : '🔴'} \${service}: \${result.status}\`
        )
      }
      ...
      const facts = results.map(r =&gt; ({
        label: r.name,
        value: \`\${r.ok ? '🟢' : '🔴'} \${r.status} (\${r.latency}ms)\`
      }))
      ...
      return createMessageResponse({
        ...
        blocks: [
          { type: 'section', text: allOk ? '🟢 **All systems operational**' : '⚠️ **Some services degraded**' },
...
<h2 id="secrets">Secret Rotation Reminder</h2>

<pre data-lang="js"><code>// modules/secrets.js
import { Command, createTextResponse, createMessageResponse } from '@devchitchat/chatopsjs'
...
export default async function (robot) {
  ...
  robot.commands.register(new Command({
    ...
    confirm: {
      mode:    'yes-no',
      message: '🔒 Rotate this secret? Old key will be invalidated immediately. (yes/no)'
    },
    handler: async (ctx) =&gt; {
      const { service, key } = ctx.args

      // Log the rotation attempt — never log the value!
      ...
      // Call your secrets manager API here
      // const newKey = await rotateSecret(service, key)

      return createMessageResponse({
        fallbackText: \`Secret rotated: \${service}/\${key}\`,
        blocks: [
          { type: 'section', text: \`🔒 **Secret rotated**\` },
          { type: 'facts',   items: [
            { label: 'Service',    value: service },
            { label: 'Key',        value: key     },
  ...
  robot.commands.register(new Command({
    ...
    handler: async (ctx) =&gt; {
      ...
      const lines = log.slice(-10).map(e =>
        \`\${e.timestamp} · \${e.service}/\${e.key} by \${e.rotatedBy}\`
      )
